...
crontab_update_start() {
    ...
    echo -e "\n0 7 * * * /bin/bash <(wget -qO- https://fastly.jsdelivr.net/gh/P3TERX/aria2_perfect_config/tracker.sh) ${aria2_conf} RPC 2>&1 | tee ${aria2_conf_dir}/tracker.log" >>"/tmp/crontab.bak"
...
Update_bt_tracker() {
    ...
    check_pid
    [[ -z $PID ]] && {
        bash <(wget -qO- https://fastly.jsdelivr.net/gh/P3TERX/aria2_perfect_config/tracker.sh) ${aria2_conf}
    } || {
        bash <(wget -qO- https://fastly.jsdelivr.net/gh/P3TERX/aria2_perfect_config/tracker.sh) ${aria2_conf} RPC
    }
}
Update_aria2() {
    check_installed_status
